...
int main() {
	int fd_r = ft_popen("ls", (char *const[]){"ls", "-C", NULL}, 'r');
	if (fd_r == -1) {
		printf("ls test failed, returned -1\n");
		return (0);
	}
	char buf[1];
	...
	int fd_w = ft_popen("cat", (char *const[]){"cat", NULL}, 'w');
	if (fd_w == -1) {
		printf("cat test failed, returned -1\n");
		return (0);
	}
	dprintf(fd_w, "test\n");
	close(fd_w);

	int fd_rc = ft_popen("cat", (char *const[]){"cat", "popen.c", NULL}, 'r');
	if (fd_rc == -1) {
		printf("cat test failed, returned -1\n");
		return (0);
	}
	while (read(fd_rc, buf, 1))
		write(1, buf, 1);
	close(fd_rc);
	return (0);
}